...
const fetchData = async () => {
    ...
    try {
        const response = await fetch(`https://counpaign.com/api/dashboard/points-details?businessId=${authStore.user.businessId}`);
        if (!response.ok) return;
        const data = await response.json();
        transactions.value = data;
        totalPoints.value = data.reduce((sum: number, tx: PointTransaction) => sum + tx.points, 0);
    } catch (error) {
...
<template>
    <div class="detail-page p-4">
        <div class="flex align-items-center gap-3 mb-4">
            ...
            <div>
                <h1 class="text-2xl font-bold m-0">Kazanılan Puanlar</h1>
                <p class="text-secondary m-0">Müşterilerin kazandığı puan detayları</p>
            </div>
        </div>
        ...
        <div class="stats-card mb-4">
            <div class="flex align-items-center gap-3">
                <div class="bg-purple-100 border-circle p-3">
                    <i class="pi pi-star text-purple-500 text-2xl"></i>
                </div>
                <div>
                    <span class="text-500 text-sm">Toplam Kazanılan Puan</span>
                    <div class="text-900 font-bold text-3xl">{{ totalPoints.toLocaleString('tr-TR') }}</div>
                </div>
            </div>
        </div>
            ...
            >
                <template #empty>
                    <div class="text-center py-5 text-secondary">
                        ...
                        <p>Henüz puan kazanımı bulunmuyor</p>
                    </div>
                </template>
                ...
                <Column field="points" header="Puan" sortable style="min-width: 100px">
                    <template #body="{ data }">
                        <span class="font-bold text-purple-500">+{{ data.points }}</span>
                    </template>
                </Column>
                ...
                <Column field="date" header="Tarih" sortable style="min-width: 160px">
                    <template #body="{ data }">
